...
def test(model, test_loader, epoch, device, logger):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            total_test_loss.update(test_loss.item())
            prec1, prec5 = accuracy(output.cpu().detach(), target.cpu().detach(), (1, 5))
...
def main_mobile():
    # +
    csv_path = "/data/kjh/save_log/cifar100_q_resnet_cifar/log_cifar100_q_resnet_cifar_bs128_ep200_seed_3/csv/"
    ...
    option = Option("./test_mobile.hocon")
    ...
    csv_path = os.path.join(option.save_path, "resnet_kjh_test.csv")
